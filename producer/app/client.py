...
class ControllerClient:
    ...
    async def get_service_list(self) -> List[Service]:
        try:
            response = await httpx.AsyncClient().get(f"{CONTROLLER_SERVICE_URL}/service/types")
    ...
    async def get_exchange(self, service_type: str) -> ExchangeInfo:
        try:
            response = await httpx.AsyncClient().post(f"{CONTROLLER_SERVICE_URL}/client/exchange/{service_type}")
    ...
    async def get_ticket_number_and_queue(self) -> TicketInfo:
        ...
        try:
            response = await httpx.AsyncClient().post(f"{CONTROLLER_SERVICE_URL}/ticket?client_id={self.client_id}")
            data = response.json()
