...
# --- Internal State & Kubernetes Client ---
# Using a simple in-memory cache for simplicity.
# For production-grade resilience, consider storing state in a ConfigMap or custom resource.
...
def _get_k8s_clients():
    ...
    if _k8s_co_api is None or _k8s_apps_api is None:
        try:
            ...
            configuration = client.Configuration._default
...
def create_scaled_object(queue_name, target_deployment_name, logger):
    ...
    api = _get_k8s_clients()[0]
    scaled_object_name = _get_scaled_object_name(queue_name)

    # --- IMPORTANT: CUSTOMIZE THIS LOGIC ---
    # This is where you define how a queue name maps to a Kubernetes Deployment name.
    # The example uses a simple prefix/suffix removal. Adapt this to your actual naming conventions.
    # Example: 'my-app-queue-user123' -> 'my-app-consumer-user123'
    ...
    try:
        _get_k8s_clients()[1].read_namespaced_deployment(name=target_deployment_name, namespace=OPERATOR_NAMESPACE)
        ...
        for queue_name in current_rabbitmq_queues:
            if queue_name not in managed_scaled_objects:
                ...
                if queue_name.startswith('service.request'):
                    target_deployment_name = queue_name.removeprefix('service.request.')
                else:
                    logger.warning(f"No specific mapping rule for queue '{queue_name}'. Skipping creation.")
                    target_deployment_name = None # Indicate no mapping
